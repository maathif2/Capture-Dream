        ...
        <div class="login_Box">
            <div class="box">
                ...
                if($_SERVER['REQUEST_METHOD'] == "POST"){
                    ...
                    $pass = $_POST['psw'];
                    $password =$pass;
                    $sql = "SELECT * FROM photographer WHERE p_name='$username' AND p_pass='$password'";
                    $result = $con->query($sql);
    
                    if($result->num_rows > 0){
                        //login success
                        $row=$result->fetch_assoc();
                        $_SESSION["photo_id"] = $row['photo_id']; //set session
                        header("Location:../Photographeraccount/PhotographerAccount.php");
                    }else{
                        //login failed
